...
>
	<div class="text-center">
		<h1
			...
			class="font-ginto mb-18 text-5xl font-black tracking-wide text-white sm:text-7xl md:text-8xl"
		>
			Built With
		</h1>
	</div>
	...
	<div class="grid grid-cols-2 gap-6 sm:grid-cols-3 md:gap-8 lg:grid-cols-4">
		{#each techStack as tech, i}
			<div
				...
				class="group flex h-32 w-32 flex-col items-center justify-center rounded-xl bg-gradient-to-br from-[#2c333c] to-[#1e252b] p-4 opacity-0 shadow-lg transition-all duration-300 hover:scale-110 hover:shadow-xl sm:h-40 sm:w-40"
				style="--tech-color: {tech.color}"
			>
				<div
					class="mb-3 text-gray-300 transition-colors duration-300 group-hover:text-white"
					style="color: var(--tech-color)"
				>
					{@html icons[tech.icon]}
				</div>
				<h3 class="font-walsheim-regular text-center text-xs font-semibold text-white sm:text-sm">
					{tech.name}
				</h3>
				<p
					class="font-walsheim-regular mt-1 text-center text-xs text-gray-400 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
				>
					{tech.description}
				</p>
			</div>
	...
	<div class="font-walsheim-regular text-center text-sm text-gray-400">
		<p class="font-walsheim-regular mx-auto mt-18 max-w-2xl text-base text-gray-300 sm:text-lg">
